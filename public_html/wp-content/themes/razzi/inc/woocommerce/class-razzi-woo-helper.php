...
class Helper {
	...
	public static function get_product_video() {
		...
		if ( $video_html ) {
			$vid_html   = '<div class="razzi-video-wrapper ' . esc_attr( $video_class ) . '">' . $video_html . '</div>';
			$video_html = '<div data-thumb="' . esc_url( $video_thumb ) . '" class="woocommerce-product-gallery__image razzi-product-video">' . $vid_html . '</div>';
		}
